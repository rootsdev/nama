...
def get_clusters_from_table(names_to_cluster,
                            clustered_names,
                            verbose=True):
...
def write_clusters(path, name_cluster):
    df = pd.DataFrame([{"name": remove_padding(name), "cluster": cluster_id} for name, cluster_id in name_cluster.items()])
...
def get_names_to_cluster(name_freq, n_to_cluster):
    return np.array([name for name in name_freq.keys() if len(name) > 1][:n_to_cluster])


def get_distances(name_freq,
                  names_to_cluster,
                  swivel_model,
                  swivel_vocab,
                  tfidf_vectorizer,
...
def generate_clusters_from_distances(cluster_algo,
                                     cluster_linkage,
                                     cluster_threshold,
                                     distances,
                                     names_to_cluster,
    ...
    model, clusters = generate_clusters(distances,
...
def get_validation_results(input_names_eval,
    ...
    search_thresholds = search_threshold if type(search_threshold) == list else [search_threshold]
    ...
    for size in validation_sizes:
        ...
        if lookup_mode:
            name2clusters, cluster2names = get_clusters_from_table(all_names_validate,
                                                                   name_cluster,
                                                                   verbose=verbose)
        else:
        ...
        best_matches = get_best_cluster_matches(name2clusters,
                                                cluster2names,
