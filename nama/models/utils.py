...
def _get_candidate_scores(shared, rows, _):
    ...
    if metric == "cosine":
        # If vectors are normalized dot product and cosine similarity are the same
        scores = safe_sparse_dot(rows, source_names_X.T) if normalized else cosine_similarity(rows, source_names_X)
        # partition and sort
...
def get_best_matches(
    ...
    if n_jobs == 1:
        ...
        for batch, ix in batches:
            results.append(
                _get_candidate_scores((source_names, source_names_X, num_candidates, metric, normalized), batch, ix)
            )
        candidate_names_scores = np.vstack(results)
    else:
        with WorkerPool(
            shared_objects=(source_names, source_names_X, num_candidates, metric, normalized), n_jobs=n_jobs
        ) as pool:
