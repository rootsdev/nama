...
from nama.models.swivel_encoder import convert_names_to_model_inputs
from nama.models.utils import ArrayDataLoader, add_padding, check_convert_tensor, get_best_matches, remove_padding


class SwivelDataset:
    def __init__(self, input_names, record_name_frequencies, vocab_size=200000, symmetric=True):
        # get the most-frequent names from record_name_frequencies
        ...
        for input_name, rnf in zip(input_names, record_name_frequencies):
            ...
            for name, frequency in rnf:
                ...
                dok[name_id, input_id] += frequency
                dok[input_id, input_id] += frequency
                if symmetric:
                    dok[input_id, name_id] += frequency
                    dok[name_id, name_id] += frequency
...
class SwivelModel(nn.Module):
    ...
    def init_params(self, row_sums, col_sums, embeddings=None):
        ...
        self.bi.weight.data = torch.unsqueeze(torch.tensor([math.log(x + 1) for x in row_sums], dtype=torch.float32), 1)
        self.bj.weight.data = torch.unsqueeze(torch.tensor([math.log(x + 1) for x in col_sums], dtype=torch.float32), 1)

    def forward(
        self, row_indices: torch.tensor, col_indices: torch.tensor, co_occurrences: torch.tensor, matrix_log_sum: float
    ):
        ...
        l2_confidence = self.confidence_base + self.confidence_scale * torch.pow(
            co_occurrences, self.confidence_exponent
        )
...
def train_swivel(model, dataset, n_steps=100, submatrix_size=1024, lr=0.05, device="cpu", optimizer=None, verbose=True):
    ...
    if n_steps > 0:
        ...
        shard_positions = [
            (random.randrange(0, n_shards_row), random.randrange(0, n_shards_col))  # noqa: S311
    ...
    for step, shard_position in enumerate(shard_positions):
        ...
        if verbose and step % 10000 == 0:
            print(f"Step: {step}/{len(shard_positions)} \t Loss: {np.mean(loss_values[-1000:])} - {datetime.now()}")
...
def get_best_swivel_matches(
    ...
    input_name_embeddings = get_swivel_embeddings(
        model, vocab, input_names, add_context=add_context, encoder_model=encoder_model
    )

    # Get embeddings for candidate names
    candidate_name_embeddings = get_swivel_embeddings(
        model, vocab, candidate_names, add_context=add_context, encoder_model=encoder_model
    )
...
def write_swivel_embeddings(path, names, embeddings):
    data = (
        "\n".join([
            json.dumps({"name": remove_padding(name), "embedding": embedding.tolist()})
            for name, embedding in zip(names, embeddings)
        ])
    ).encode("utf-8")
    with gzip.open(open(path, "wb"), "wb") as f:
        f.write(data)


def read_swivel_embeddings(path):
    with gzip.open(open(path, "rb"), "rb") as f:
        data = f.read()
    names = []
    embeddings = []
