...
import jellyfish
import numpy as np
from src.models.levenshtein import get_best_lev_matches, get_lev_scores
...
def _get_ensemble_names_scores(
    ensemble_model,
    name_freq,
    input_names,
    k,
...
def get_best_ensemble_matches(
    ...
    if np.sum(input_in_vocab_ixs) > 0:
        ...
        if n_candidate_in_vocab > 0:
            swivel_names_scores_in_vocab = get_best_swivel_matches(
                model=model,
                vocab=vocab,
            ...
            lev_scores_in_vocab = get_lev_scores(
        ...
        if n_candidate_out_vocab > 0:
            lev_names_scores_candidate_out_vocab = get_best_lev_matches(
                tfidf_vectorizer,
                input_names[input_in_vocab_ixs],
        ...
        ensemble_names_scores[input_in_vocab_ixs] = _get_ensemble_names_scores(
            ensemble_model,
            name_freq,
            input_names[input_in_vocab_ixs],
            k,
    ...
    if np.sum(input_out_vocab_ixs) > 0:
        ...
        lev_names_scores_input_out_vocab = get_best_lev_matches(
            tfidf_vectorizer,
        ...
        ensemble_names_scores[input_out_vocab_ixs] = _get_ensemble_names_scores(
            ensemble_model,
            name_freq,
            input_names[input_out_vocab_ixs],
            k,
