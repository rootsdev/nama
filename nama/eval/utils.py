...
def similars_to_ndarray(similar_names_scores: list[list[tuple]]) -> np.ndarray:
    ...
    names = np.array([[cell[0] for cell in row] for row in similar_names_scores], dtype="O")
    # scores is a 2D array [names, score of k similar-names]
    scores = np.array([[cell[1] for cell in row] for row in similar_names_scores], dtype="f8")
...
def levenshtein(seqs1, seqs2):
    ...
    for x in range(1, max_len + 1):
        for y in range(1, max_len + 1):
            matrix[x, y] = np.minimum(
                np.minimum(matrix[x - 1, y] + 1, matrix[x - 1, y - 1] + cost[x - 1, y - 1]), matrix[x, y - 1] + 1
            )
